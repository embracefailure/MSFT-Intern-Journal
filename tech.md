...
Apache Spark内存计算的优势：
1. **速度**：Spark将数据加载到内存中进行处理，避免了频繁的磁盘I/O操作，大大提高了计算速度。
2. **迭代计算**：Spark特别适合需要多次迭代的算法（如机器学习），因为它可以将中间结果保存在内存中，避免重复计算。

Apache Spark的核心组件：
- **Spark Core**：提供了基本的分布式任务调度和内存管理功能。
- **Spark SQL**：用于处理结构化数据，支持SQL查询和DataFrame API。
- **Spark Streaming**：用于实时数据处理，支持流式数据的处理和分析。
- **MLlib**：Spark的机器学习库，提供了常用的机器学习算法和工具。

Apache Spark与cuda的关系：

Spark 是一个分布式计算框架，用于**大规模数据处理**，而 CUDA 是专为 GPU 提供加速计算的平台。

Spark 本身不直接使用 CUDA，但可以通过集成 CUDA 加速 GPU 密集型任务，尤其是在深度学习和大规模计算中。

Spark + CUDA 的组合允许在分布式集群中高效处理大规模数据，并在需要大量并行计算的任务（如深度学习训练）中显著提高性能。
...
3.streaming模式：如果使用流式输出，TTPS会更高，因为可以在生成过程中逐步返回结果，而不是等待整个响应完成。
4. batch size：批量处理多个请求可以提高TTPS，因为可以更高效地利用计算资源。
5. 硬件：A100< H100 < TPUv5
